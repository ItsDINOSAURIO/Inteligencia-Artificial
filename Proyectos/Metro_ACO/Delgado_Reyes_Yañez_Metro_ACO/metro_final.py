...
class metro:
    ...
    def mostrar_lista_nodos(self, accion):
        ...
        boton_aplicar = tk.Button(
            ...
            command=lambda: [self.actualizar_estaciones(accion), self.actualizar_lista_principal()],
            bg="purple",
            fg="white",
            font=("Arial", 10),
        )
    ...
    def actualizar_lista_principal(self):
        ...
        self.lista_fin = [
            self.estaciones[nodo] for nodo, estado in self.estados_nodos.items() if estado == "inhabilitado"
        ]

        for estacion in self.lista_inicio:
            self.lista1.insert(tk.END, estacion)

        for estacion in self.lista_fin:
            ...
            for hormiga in range(num_hormigas):
                ...
                while nodo_actual != self.nodo_out:
                    nodos_vecinos = [
                        ...
                        and nodo not in nodos_visitados and self.estados_nodos[nodo]=="habilitado"
